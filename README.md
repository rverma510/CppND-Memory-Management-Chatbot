...
This is the Memory Management project.
...
* wxWidgets >= 3.0
  * Linux: First install gtk `sudo apt install libgtk-3-dev`. Then download source for wxwidgets from `https://www.wxwidgets.org/downloads/`. After Downloading, unzip the source code and run the following commands in order `./configure`, `make`, `sudo make install` and `sudo ldconfig`.
